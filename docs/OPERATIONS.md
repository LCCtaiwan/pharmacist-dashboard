...
目前正式方案由 GitHub Actions 負責採集，GitHub Pages 與 GAS Web App 共用結果；GAS 另外負責 Google Sheet 月份同步，不負責網站採集。
...
1. GAS 專案已部署為 Web App 第 17 版，存取權限為「所有人」。這個 GAS 儀表板與 Sheet 都讀取 Actions 同步結果，不執行網站採集。
